    ...
    */
    ...
    {
        let cmd = util.type.isArray(calls) ? [] : {}, cnt = 0, cb = function(cmd){
            ajax.batch(cmd, callback, bHaltOnError);
        };

        for(let i in calls)
        {
            let method = null, params = null;
    ...
    BX24.loadScript = function(script, callback)
    {
        ...
        let load_js = function(ind)
        {
            ...
            if(!!script[ind])
            {
                let oHead = document.getElementsByTagName("HEAD")[0] || document.documentElement;
    ...
    let util = {
        ...
        delegate: function (func, thisObject)
        {
            ...
            return function() {
                let cur = proxyContext;
                proxyContext = this;
                let res = func.apply(thisObject, arguments);
                proxyContext = cur;
                return res;
            }
        },
...
// communication with parent frame utility
    let sendMessage = function(cmd, params, cb)
    {
        if(util.type.isFunction(params))
        {
        ...
        if (cmd.indexOf(':') >= 0)
        {
            fetch('http'+(PARAMS.PROTOCOL?'s':'')+'://' + PARAMS.DOMAIN,
                {
                    method: cmd,
        ...
        else
        {
            ...
            xhr.onload = function()
            {
                ...
                if(bSuccess)
                {
                    ...
                    {
                        let res = new ajaxResult(data, config, s);
            ...
            let query_data = 'auth=' + PARAMS.AUTH_ID;
    ...
    let ajaxResult = function(answer, query, status)
    {
        this.answer = answer;
    ...
    let ajaxError = function(status, ex)
    {
        this.status = status;
        this.ex = ex;
    };
    ...
    let fileReader = function(fileInput, cb)
    {
        if(fileReader.canUse())
        {
            ...
            for (i = 0, f; f = files[i]; i++)
            {
                ...
                reader.onload = function(e){
                    ...
                    let res = [this.BXFILENAME,btoa(e.target.result)];
    ...
    let RestrictionManager = function()
    {
        let _ = this;
...
// AUTH_ID can also be set via GET, but it's not recommended due to security reasons
    ...
    let PARAMS = {
        ...
        PROTOCOL: 1,
        APP_SID: process.env.REACT_APP_CLIENT_ID,
        PATH: '',
        LANG: '',
        AUTH_ID: '',
        REFRESH_ID: null,
